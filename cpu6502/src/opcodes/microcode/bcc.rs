...
mod unit_tests {
    ...
    // Test core BCC microcode functions
    #[test]
    fn test_fetch_offset_carry_clear_positive_offset() {
        let mut cpu = create_test_cpu_with_data(&[0x10], 0x1000); // Positive offset (+16)
        cpu.registers.program_counter = 0x1000;
        cpu.flags.carry = false; // Carry clear - should branch
    ...
    #[test]
    fn test_fetch_offset_carry_clear_negative_offset() {
    ...
    #[test]
    fn test_fetch_offset_carry_set_no_branch() {
        let mut cpu = create_test_cpu_with_data(&[0x20], 0x1000);
        cpu.registers.program_counter = 0x1000;
        cpu.flags.carry = true; // Carry set - should not branch
    ...
    #[test]
    fn test_bcc_alu_error_handling() {
        let mut cpu = create_test_cpu();
        cpu.registers.program_counter = 0x0000; // Edge case that might cause ALU error
        cpu.temp_data = 0x80; // -128

        // This should test error handling, though in practice the ALU function
        // should handle all valid PC + offset combinations
    ...
    // Test complete BCC instruction sequences
    #[test]
    fn test_bcc_relative_branch_taken_no_page_cross() {
        let mut cpu = create_test_cpu_with_data(&[0x10], 0x1000); // +16 offset
        cpu.registers.program_counter = 0x1000;
        cpu.flags.carry = false; // Branch should be taken
    ...
    #[test]
    fn test_bcc_relative_branch_taken_with_page_cross() {
    ...
    #[test]
    fn test_bcc_relative_branch_not_taken() {
        let mut cpu = create_test_cpu_with_data(&[0x20], 0x1000); // +32 offset
        cpu.registers.program_counter = 0x1000;
        cpu.flags.carry = true; // Branch should NOT be taken
    ...
    #[test]
    fn test_bcc_negative_branch_with_page_cross() {
    ...
    #[test]
    fn test_bcc_edge_case_combinations() {
        let test_cases = [
    ...
    #[test]
    fn test_bcc_preserves_other_flags() {
    ...
    #[test]
    fn test_bcc_bus_error_propagation() {
    ...
    #[test]
    fn test_bcc_complete_instruction_simulation() {
        // Test a realistic BCC instruction in different scenarios

        // Scenario 1: Simple forward branch taken
        let mut cpu = create_test_cpu_with_data(&[0x08], 0x2000); // BCC +8
        ...
        let mut cpu = create_test_cpu_with_data(&[0xF8], 0x2000); // BCC -8
        cpu.registers.program_counter = 0x2000;
        ...
        let mut cpu = create_test_cpu_with_data(&[0x80], 0x2100); // BCC -128
